...
{
    return number_format($point, 2).'점';
}
        ...
        if($cp['cp_id']) {
            ...
            if($cp['cp_minimum'] <= $tot_od_price) {
                if($cp['cp_type']) {
                    ...
                    $dc = (floor((($tot_od_price * ($cp['cp_price'] / 100)) / $cp['cp_trunc'])*100)/100) * $cp['cp_trunc'];
                } else {
                    $dc = $cp['cp_price'];
                }
        ...
        if($cp['cp_id']) {
            ...
            if($cp['cp_minimum'] <= $tot_od_price) {
                if($cp['cp_type']) {
                    ...
                    $dc = (floor((($send_cost * ($cp['cp_price'] / 100)) / $cp['cp_trunc'])*100)/100) * $cp['cp_trunc'];
                } else {
                    $dc = $cp['cp_price'];
                }
...
function get_item_point($it, $io_id='', $trunc=10)
{
    ...
    if($it['it_point_type'] > 0) {
        ...
        $it_point = (floor((($it_price * ($it['it_point'] / 100) / $trunc))*100)/100) * $trunc;
    } else {
